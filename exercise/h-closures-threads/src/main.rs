...
fn main() {
    ...
    // variable.  Uncomment the println.  If you did 1a and 1b correctly, the sum should be 35.
    //
    let sum = handle.join().expect("Error");
    ...
    for num in 0..10 {
        pause_ms(100);
        tx.send("Main thread: Sending message ".to_string() + &num.to_string())
            .unwrap();
    }
    drop(tx);
    handle_a.join().unwrap();
